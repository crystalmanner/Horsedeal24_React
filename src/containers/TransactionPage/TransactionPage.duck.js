...
export const acceptSale = (id, providerData) => (dispatch, getState, sdk) => {
  // here
  ...
  return sdk.transactions
    .transition({ id, transition: TRANSITION_ACCEPT, params: { protectedData: { providerData: JSON.stringify(providerData) }} }, { expand: true })
    .then(response => {
      dispatch(addMarketplaceEntities(response));
...
const fetchMessages = (txId, page) => (dispatch, getState, sdk) => {
  ...
  return sdk.messages
    ...
    .then(async response => {
      ...
      try {
        const filesJson = await fetch(`${origin}/api/transactions/${transactionUuid}/messages`)
        ...
        console.error('The following error occured at fetchMessages method: ', e)
      }
...
export const sendFile = (txId, file) => (dispatch, getState, sdk) => {
  const transactionUuid = txId.id.uuid
  const formData = new FormData();

  formData.append('file', file)
  formData.append('content', '__file')

  dispatch(sendMessageRequest()); 

  return fetch(`${origin}/api/transactions/${transactionUuid}/messages?page=1`, {
    method: 'POST',
    body: formData
  })
    .then(res => res.json())
    .then(res => {
      return dispatch(fetchMessages(txId.id, 1))
        .then(() => {
          dispatch(sendMessageSuccess());
          return res;
        })
