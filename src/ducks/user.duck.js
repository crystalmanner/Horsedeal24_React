...
export const fetchCurrentUser = (params = null, additionalData = null) => (dispatch, getState, sdk) => {
  ...
  return sdk.currentUser
    ...
    .then(async response => {
      ...
      const FBPictureAlreadySet = currentUser.attributes.profile.publicData.FBPicture
      const additionalDataSet = additionalData && additionalData.FBPicture

      if(additionalDataSet && !FBPictureAlreadySet) {
        const FBPicture = `https://graph.facebook.com/${additionalData.FBPicture}/picture?type=large`
        currentUser.attributes.profile.publicData.FBPicture = FBPicture
        await sdk.currentUser.updateProfile({ publicData: { FBPicture }})
      }
